...
class ProjectView(ctk.CTkFrame):
    def __init__(
        ...
        topbar.grid(row=0, column=0, sticky="ew", padx=28, pady=(18, 0))
        ctk.CTkButton(
        ...
        ).grid(row=1, column=0, sticky="ew", padx=32, pady=(14, 2))
        ...
        ).grid(row=2, column=0, sticky="ew", padx=32, pady=(0, 14))

        stats_frame = ctk.CTkFrame(self, fg_color="transparent")
        stats_frame.grid(row=3, column=0, sticky="ew", padx=32, pady=(0, 10))
        ...
        ctk.CTkButton(
            self, text="\u25B6  Work", height=46, width=260, font=ctk.CTkFont(size=16, weight="bold"),
            command=on_work,
        ).grid(row=4, column=0, pady=(6, 4))
        ...
        ctk.CTkLabel(self, text=last_line, text_color="gray55", font=ctk.CTkFont(size=12)).grid(
            row=5, column=0, pady=(2, 8)
        )

        history = ctk.CTkScrollableFrame(self, fg_color="transparent")
        history.grid(row=6, column=0, sticky="nsew", padx=28, pady=(4, 18))
    ...
    def _build_contributions(self, parent, stats) -> None:
        ...
        items = self._projects.contributions(self._project_id, limit=8)
        if not items:
            ctk.CTkLabel(parent, text="Nothing logged yet.", text_color="gray55").grid(
        ...
        for contribution in items:
            ...
            label = ctk.CTkLabel(
                item, text=bullet, font=ctk.CTkFont(size=13), anchor="w", wraplength=700, justify="left"
            )
            label.pack(anchor="w")
